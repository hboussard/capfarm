...
public class OnBooleanConditionConstraint extends CoverAllocationConstraint<Boolean, Parcel> {

	private static final long serialVersionUID = 1L;
	
	public OnBooleanConditionConstraint(String code, boolean checkOnly, ConstraintMode mode, Set<Cover> covers, Set<Parcel> location, Domain<Boolean, Parcel> domain) {
		super(code, checkOnly, ConstraintType.OnBooleanCondition, mode, covers, location, domain);
	}
	
	protected boolean condition(Parcel p){
		return ((VariableBooleanDomain) domain()).accept(p);
	}
	...
	public boolean check(Instant start, Instant end, boolean verbose){
		...
		sb.append("on boolean condition ");
